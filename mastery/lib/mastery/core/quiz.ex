defmodule Mastery.Core.Quiz do
  ...
  def new(fields) do
    # create new Quiz struct from fields - where fields is a keyword list
  ...
  defp remove_template_from_category(quiz) do
    ...
    # fetch templates for template.category from quiz.templates
    # delete template the list
    new_category_templates =
      quiz.templates
      |> Map.fetch!(template.category)
      |> List.delete(template)

    # since List.delete returns the new list after deletion
    # check if the list ends up being empty, and either delete the category
    # or put replace the templates list of the category with the updated list
    new_templates =
